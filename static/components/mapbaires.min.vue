...
/* harmony default export */ __webpack_exports__["a"] = ({
  ...
  mounted: function mounted() {
    var vue_ref = this; // Set svg width & height
    ...
    svg.append('rect').attr('class', 'background').attr('width', size.width).attr('height', size.height).on('click', clicked);
    var g = svg.append('g');
    ...
    d3.json(geoJsonUrl, function (error, mapData) {
      ...
      color.domain([0, d3.max(features, nameLength)]); // Draw each province as a path
    ...
    function clicked(d) {
      var x, y, k; // Compute centroid of the selected path
      ...
      g.transition().duration(750).attr('transform', 'translate(' + size.width / 2 + ',' + size.height / 2 + ')scale(' + k + ')translate(' + -x + ',' + -y + ')');
    }
    ...
    function mouseout(d) {
      vue_ref.selectProvince(undefined); // Reset province color
  ...
  methods: {
    ...
    openInfo: function openInfo(province) {
